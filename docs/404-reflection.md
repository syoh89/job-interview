...
## To-do 5: 배포 후 상세/키워드 URL 200 확인

### 잘한 점

- 최신 Pages artifact를 확인해 정적 HTML이 정상 생성되는지 선확인 후, 실제 배포 URL을 다시 검증함.
- `/studies/http-protocol/`, `/keywords/HTTP/`에서 콘텐츠가 노출되는 것을 확인함.

### 실수/개선점

- 배포 직후 캐시/전파 지연을 고려하지 않고 너무 빠르게 404를 판단했음.
